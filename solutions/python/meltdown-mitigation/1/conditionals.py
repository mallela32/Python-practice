...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
    effeciancy_percenatage = int((voltage * current / theoretical_max_power) * 100)
    if effeciancy_percenatage >= 80:
        return 'green'
    elif effeciancy_percenatage >= 60 and effeciancy_percenatage < 80:
        return 'orange'
    elif effeciancy_percenatage >= 30 and effeciancy_percenatage < 60:
        return 'red'
    return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    current_danger = temperature * neutrons_produced_per_second
    current_range = threshold * 90 / 100
    min_range = threshold - (threshold * 10/100)
    max_range = threshold + (threshold * 10/100)
    if  current_danger < current_range :
        return 'LOW'
    elif current_danger >= min_range and current_danger <= max_range:
        return 'NORMAL'
    return 'DANGER'
